...
const EmojiPoint = ({ point }: { point: string }) => {
  ...
  return (
    <div className="group relative bg-linear-to-br from-gray-200/[0.08] to-gray-400/[0.03] p-4 rounded-2xl border border-gray-500/10 hover:shadow-lg transition-all ">
      <div className="absolute inset-0 bg-linear-to-r from-gray-500/10 to-transparent opacity-0 group-hover:opactiy-100 transition-opacity rounded-2xl" />
    ...
    <div className="space-y-4">
      {points.map((point, ind) => {
        ...
        if (hasEmoji || isMainPoint) {
          return <EmojiPoint key={`point-${ind}`} point={point} />;
        }
        return <RegularPoint key={`point-${ind}`} point={point} />;
      })}
    </div>
  );
};
